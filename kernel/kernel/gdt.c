...
static void add_r3_gdt_code_item(int gdt_index, int base, int limit) {
...
static void add_r3_gdt_data_item(int gdt_index, int base, int limit){
    // 在实模式时已经构建了4个全局描述符，所以从4开始
    if (gdt_index < 4) {
        printk("the gdt_index:%d has been used...\n", gdt_index);
        return;
...
void gdt_init(){
    ...
    // 创建r3用的段描述符：代码段、数据段
    add_r3_gdt_code_item(4, 0, 0xfffff);
    add_r3_gdt_data_item(5, 0, 0xfffff);
