...
// 通过索引寄存器选择寄存器，通过数据寄存器向选择的寄存器读取或写入数据
...
static uint screen; // 当前显示器开始的内存位置 （字节count， （row×80 + col）×2）
static uint pos; // 记录当前光标的内存位置 （字节count， （row×80 + col）×2）
static uint x, y; // 当前光标的坐标（字count， x = row， y = col）
